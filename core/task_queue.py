...
class TaskQueueManager:
            ...
            for node in workflow_data['nodes']:
                ...
                for input_name, input_data in node.get('inputs', {}).items():
                    if isinstance(input_data, dict) and '$ref' in input_data:
                        ...
                        # 注意：这里需要确保ref_node_id已经处理过，所以需要按依赖顺序处理节点
    ...
    # 兼容旧接口
    def enqueue_workflow(self, workflow_data: Dict[str, Any], retry_config: Optional[Dict[str, Any]] = None) -> str:
        """将工作流任务添加到队列
        ...
        task_id = asyncio.run(self.add_workflow(workflow_id, workflow_data))
        return task_id
    ...
    def is_redis_available(self) -> bool:
        ...
        Returns:
            始终返回False，因为使用ExecutionQueue不需要Redis
        """
        return False
