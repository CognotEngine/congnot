...
def init_hunyuan_dependencies():
    """Initialize HunyuanVideo dependencies"""
    global HUNYUAN_VIDEO_AVAILABLE, torch, HunyuanVideoPipeline, DEVICE

    if HUNYUAN_VIDEO_AVAILABLE is not None:
        return HUNYUAN_VIDEO_AVAILABLE
    ...
    try:
        ...
        print(f"HunyuanVideo using device: {DEVICE}")

        HUNYUAN_VIDEO_AVAILABLE = True
        return True
    except ImportError as e:
        print(f"HunyuanVideo dependencies not installed: {e}")
        HUNYUAN_VIDEO_AVAILABLE = False
        return False


@register_node(
    name="hunyuan_text_to_video",
    description="Generate videos using HunyuanVideo text-to-video model",
    category="ai",
    icon="🎥"
)
class HunyuanTextToVideoNode(BaseNode):
    ...
    def __call__(self, prompt: str = "一只猫对着镜头笑",
        ...
        if not init_hunyuan_dependencies():
            return {
                "video_path": "",
                "success": False,
                "message": "HunyuanVideo dependencies not installed. Please install them using: pip install -r requirements.txt"
            }
        ...
        try:
            ...
            if text_to_video_pipeline is None:
                print("Loading HunyuanVideo text-to-video pipeline...")
                text_to_video_pipeline = HunyuanVideoPipeline.from_pretrained(
...
@register_node(
    ...
    description="Generate videos using HunyuanVideo image-to-video model",
    category="ai",
    icon="🖼️🎥"
)
class HunyuanImageToVideoNode(BaseNode):
    ...
    def __call__(self, image: str = "",
        ...
        try:
            ...
            if img_to_video_pipeline is None:
                print("Loading HunyuanVideo-I2V image-to-video pipeline...")
                img_to_video_pipeline = HunyuanVideoPipeline.from_pretrained(
